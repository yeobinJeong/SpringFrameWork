...
public class FileController {
	...
	public String upload(MultipartHttpServletRequest req, String title, String uploader, String content) {// 파일 업로드를 포함하는
		...
		if (file != null && file.getSize() > 0) {
			String savedFileName = Util.getUniqueFileName(path, file.getOriginalFilename());
			try {
				FileOutputStream ostream = new FileOutputStream(path + "/" + savedFileName);
	...
	public ModelAndView download(String uploadfileno) {

		int iUploadFileNo = Integer.parseInt(uploadfileno);
		UploadFile uploadFile =	dao.getUploadFileByUploadFileNo(iUploadFileNo);
		
		
		ModelAndView mav = new ModelAndView();
		DownloadView dv = new DownloadView();
		dv.setFileName(uploadFile.getSavedFileName());
		mav.setView(dv);
		return mav;
	}
